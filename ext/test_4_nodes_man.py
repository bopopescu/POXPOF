...
class TestVxlan (EventMixin):
    ...
    def _handle_ConnectionUp (self, event):
        ...
        if event.dpid == swith_dpid_map.get("USTC"):
            #core.PofManager.set_port_of_enable(event.dpid, 0x10041)
            #core.PofManager.set_port_of_enable(event.dpid, 0x10043)
            cooper_loop_ustc(event)
        """
        if event.dpid == swith_dpid_map.get("CNIC"):
            core.PofManager.set_port_of_enable(event.dpid, 0x20000)
            core.PofManager.set_port_of_enable(event.dpid, 0x20002)
        ...
        if event.dpid == swith_dpid_map.get("HUAWEI"):
            core.PofManager.set_port_of_enable(event.dpid, 0x20001)
            core.PofManager.set_port_of_enable(event.dpid, 0x20003)
